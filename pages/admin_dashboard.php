...
} elseif (in_array($filter, ['submitted','in_review','approved','sent_to_qualtrics','fulfilled'], true)) {
    $where[]  = 'status = ?';
    $params[] = $filter;
}
if ($search !== '') {
...
$totalCount   = $db->query('SELECT COUNT(*) FROM requests')->fetchColumn();
$pendingCount = $db->query("SELECT COUNT(*) FROM requests WHERE status IN ('submitted','in_review')")->fetchColumn();
$flaggedCount = $db->query("SELECT COUNT(*) FROM requests WHERE ai_flags IS NOT NULL AND ai_flags != ''")->fetchColumn();
...
if ($selectedId) {
    ...
    $selected = $s->fetch();
    $h = $db->prepare('SELECT * FROM status_history WHERE request_id = ? ORDER BY changed_at ASC');
...
<main class="main-content" style="padding-top:0;">
    <div class="container" style="padding-top:var(--space-xl);">
        ...
        <div class="metric-grid">
            <div class="metric-card"><p class="metric-label">Total requests</p><p class="metric-value"><?= (int)$totalCount ?></p><p class="metric-sub">All time</p></div>
            <div class="metric-card metric-warning"><p class="metric-label">Pending action</p><p class="metric-value"><?= (int)$pendingCount ?></p><p class="metric-sub">Submitted or in review</p></div>
            <div class="metric-card metric-danger"><p class="metric-label">AI flags</p><p class="metric-value"><?= (int)$flaggedCount ?></p><p class="metric-sub">Require attention</p></div>
            <div class="metric-card"><p class="metric-label">Outside service area</p><p class="metric-value"><?= (int)$outsideCount ?></p><p class="metric-sub">Auto-routed to mail</p></div>
        </div>

        <div class="filter-bar">
            <?php foreach (['all'=>'All','submitted'=>'Submitted','in_review'=>'In review','approved'=>'Approved','fulfilled'=>'Fulfilled','flagged'=>'Flagged'] as $key => $lbl): ?>
            <a href="?filter=<?= urlencode($key) ?>" class="filter-pill <?= $filter === $key ? 'active' : '' ?>"><?= $lbl ?></a>
            <?php endforeach; ?>
            <div class="search-box">
                <form method="GET"><input type="hidden" name="filter" value="<?= htmlspecialchars($filter) ?>"><input type="text" name="q" value="<?= htmlspecialchars($search) ?>" placeholder="Search events, orgs..."></form>
            </div>
        </div>
        ...
        <div class="card" style="padding:0; margin-bottom:var(--space-lg);">
            <div class="table-wrapper">
                <table>
                    <thead><tr><th>Event / requestor</th><th>Type</th><th>Status</th><th>Priority</th><th>Date</th><th>AI flag</th><th></th></tr></thead>
                    <tbody>
                    ...
                    <tr class="<?= $isSel ? 'selected' : '' ?>" onclick="location.href='?filter=<?= urlencode($filter) ?>&id=<?= $req['id'] ?>'">
                        ...
                        <td><?php if (!empty($req['ai_flags'])): ?><span class="ai-flag"><?= htmlspecialchars(substr($req['ai_flags'], 0, 28)) ?></span><?php else: ?><span class="td-muted">None</span><?php endif; ?></td>
                        <td onclick="event.stopPropagation();"><a href="?filter=<?= urlencode($filter) ?>&id=<?= $req['id'] ?>" class="btn btn-secondary btn-sm">Review</a></td>
                    </tr>
                    <?php endforeach; ?>
                    <?php endif; ?>
                    </tbody>
        ...
        <div class="card" id="detail-panel">
            ...
            <div class="alert alert-danger mb-md"><strong>AI flag:</strong> <?= htmlspecialchars($selected['ai_flags']) ?><?php if (!empty($selected['ai_classification'])): ?><br><strong>Classification:</strong> <?= htmlspecialchars($selected['ai_classification']) ?><?php endif; ?></div>
            <?php endif; ?>
